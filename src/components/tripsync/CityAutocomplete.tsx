    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-border bg-card pl-9 pr-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
        />
      </div>
      {open && results.length > 0 && (
        <ul className="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-border bg-card shadow-md">
          {results.map((r) => (
            <li
              key={r.label}
